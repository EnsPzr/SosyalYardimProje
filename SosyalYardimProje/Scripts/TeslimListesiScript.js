$(document).ready(function () {
    $.ajax({
        type: "GET",
        url: "/TeslimAlinacakBagis/TumBagiscilariGetir",
        data: "",
        dataType: "JSON",
                    ...
                    for (var i = 0; i < parseInt(result.BagisSayisi); i++) {
                        if (parseInt(i) % 2 == 0) {
                            Liste = Liste + "<tr class='even'>" +
                                "<td class='col-md-3 col-xs-3'>" + result.BagisList[i].BagisciAdiSoyadi + "</td>" +
                                "<td class='col-md-3 col-xs-3'>" + result.BagisList[i].BagisciTelNo + "</td>" +
                                "<td class='col-md-2 hidden-xs'>" + result.BagisList[i].EklenmeTarihiStr + "</td>" +
                                "<td class='col-md-2 hidden-xs'>" + result.BagisList[i].OnaylandiMiStr + "</td>" +
                                "<td class='col-md-2 col-xs-3'><a class='glyphicon glyphicon-pencil' href='/TeslimAlinacakBagis/Duzenle/" + result.BagisList[i].BagisId + "'></a> </td ></tr > ";
                        } else {
    ...
    $("#filtrele").click(function () {
        ...
        $.ajax({
            type: "GET",
            url: "/TeslimAlinacakBagis/FiltreliBagiscilariGetir",
            data: data,
            dataType: "JSON",
            contentType: "application/json; charset=utf-8",
            success: function (result) {
